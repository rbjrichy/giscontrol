...
<!-- End header -->
    ...
    <!-- Start Banner -->
    @php
        $titulo_seccion = $noticia->getNombre();//"Proyect Detail";
    @endphp
    @include('theme.currency.partials.sections.titulo_seccion')
    <!-- End Banner -->
    
    <!-- section -->
    <div id="detalle_noticia" class="container">
        <div class="p-5">&nbsp;</div>
        ...
        <div class="row pt-5">
            <div class="col-12 col-sm-6">
                @php
                    ...
                    $img_noticia = $noticia->getPrimeraImg();
                    if(is_object($img_noticia) ){
                        $image_noticia_url = asset(Storage::url('archivos_img/'.$img_noticia->ruta));
                    }
                @endphp
                    <img id="img_principal" class="img-thumbnail img-fluid" src="{{$image_noticia_url}}" alt="imagen principal">
                    <div class="img_mini_grid mt-2">
                        @foreach ($noticia->imagenes as $objImg)
                        <div class="efecto_img">
                            @php
                                $img_noticia_url = asset(Storage::url('archivos_img/'.$objImg->ruta));
                            @endphp
                            <a href="#noticia" id="img_100" link_img="{{$img_noticia_url}}" onclick="cambiar_img(this)">
                                <img src="{{$img_noticia_url}}" alt="galeria de imagenes">
                            </a>
                        </div>
                        @endforeach
                    </div>
            ...
            <div class="col-12 col-sm-6">
                <div class="text-center">
                    <h1 class="m-auto m-5">{{$noticia->getNombre()}}</h1>
